...
object equitment {
  def main(args: Array[String]): Unit = {
    ...
    val df = sQLContext.read.parquet(inputPath).select("ispid","requestmode","iseffective","putinmodeltype","isbid","isbilling",
      "iswin","winprice","adpayment","processnode","ispid","networkmannername","devicetype")

    val spark = SparkSession.builder().config(conf).getOrCreate()

    df.createTempView("log")

    val stge: DataFrame = spark.sql ( "select devicetype, " +
    ...
    val res= spark.sql("select case devicetype  when 1 then '手机' when 2 then '平板' else '其他' end as `设备`,sum(req),sum(Vreq),sum(Adrequest),sum(bidding),sum(biddingV),sum(click),sum(show),sum(winprice)/1000,sum(adpayment)/1000  from stage group by devicetype")
