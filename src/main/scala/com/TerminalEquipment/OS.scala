...
object OS {
  def main(args: Array[String]): Unit = {
    ...
    val df = sQLContext.read.parquet(inputPath).select("ispid","requestmode","iseffective","putinmodeltype","isbid","isbilling",
      "iswin","winprice","adpayment","processnode","ispid","networkmannername","devicetype","client")

    val spark = SparkSession.builder().config(conf).getOrCreate()

    df.createTempView("log")

    val stge: DataFrame = spark.sql ( "select client, " +
    ...
    val res= spark.sql("select case client  when 1 then 'android' when 2 then 'ios' else 'wp' end as `设备`,sum(req),sum(Vreq),sum(Adrequest),sum(bidding),sum(biddingV),sum(click),sum(show),sum(winprice)/1000,sum(adpayment)/1000  from stage group by client")
