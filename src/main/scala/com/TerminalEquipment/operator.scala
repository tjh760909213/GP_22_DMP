...
object operator {
  def main(args: Array[String]): Unit = {
    ...
    val df = sQLContext.read.parquet(inputPath).select("ispid","requestmode","iseffective","putinmodeltype","isbid","isbilling",
      "iswin","winprice","adpayment","processnode","ispid")

    val spark = SparkSession.builder().config(conf).getOrCreate()

    df.createTempView("log")

    val stge: DataFrame = spark.sql ( "select ispid, " +
    ...
    val res= spark.sql("select ispid,sum(req),sum(Vreq),sum(Adrequest),sum(bidding),sum(biddingV),sum(click),sum(show),sum(winprice)/1000,sum(adpayment)/1000  from stage group by ispid")
